  ...
  if (action.actionType === "prepare_application_review") {
    ...
    if (scoredJob?.description?.trim()) {
      ...
      saveTextArtifact(options, action, "proposal-copy-refresh.json", JSON.stringify({
        ...
        proposalTextPreview: regeneratedDraft.proposalText.slice(0, 1400),
        proofStrategy: regeneratedDraft.proofStrategy?.summary ?? null,
        selectedProofNames: regeneratedDraft.proofStrategy?.selectedProofNames ?? [],
        selectedPortfolioHighlights: regeneratedDraft.proofStrategy?.selectedPortfolioHighlights ?? [],
