...
export function buildBrowserSessionLaunchCommand(input: {
  ...
  return {
    ...
    args: [
      ...
      "--new-window",
      input.startUrl ?? "https://www.upwork.com",
    ],
  };
}
...
export function isCdpUnavailableError(error: unknown): boolean {
  const message = error instanceof Error ? error.message : String(error);
  return /ECONNREFUSED|connect.*9222|websocket.*failed|CDP endpoint.*unreachable|browserType\.connectOverCDP/i.test(message);
}

export function classifyBrowserSessionError(error: unknown): "browser_profile_in_use" | "cdp_unavailable" | "unknown" {
...
export async function acquireBrowserSession(
  ...
  if (options.mode === "cdp") {
    ...
    return {
      ...
      context,
      close: async () => {
        // Intentionally do not close the shared visible Chrome session.
      },
    };
  }
