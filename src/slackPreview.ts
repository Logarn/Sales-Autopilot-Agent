...
function usage(): string {
  return `Usage: npm run slack:preview -- --job-id <job-id>\n       npm run slack:preview -- --sample\n\nSends a V0 one-way Slack webhook proposal preview. Requires SLACK_CHANNEL_WEBHOOK_URL.`;
}
...
export async function runSlackPreview(args = process.argv.slice(2)): Promise<boolean> {
  ...
  if (!job) {
    console.error(`No stored job/application found for --job-id ${jobId}. Run with --sample to send a synthetic V0 preview.`);
    return false;
  }

  try {
    await sendSlackPreviewMessage({
      text: `${APP_NAME}: Slack V0 proposal preview — ${job.title}`,
      blocks: buildJobBlocks(job),
    });
    logger.info(`Slack proposal preview sent for ${sampleMode ? "synthetic sample" : `job ${job.id}`}.`);
    return true;
  } catch (error) {
