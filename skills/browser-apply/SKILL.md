...
## Purpose
Prepare browser review actions without applying or bypassing controls.
...
## Required inputs
approved job URL/apply URL, browser queue action, human notes
...
Queue opening the Upwork apply page on a VM, pause on login/2FA/CAPTCHA, and leave fields/submission to Steve.
...
Recommended next skill: `outcome-tracking`. Include the output schema above so the next skill can continue without rereading broad context.
