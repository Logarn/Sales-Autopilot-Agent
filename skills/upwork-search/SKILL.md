...
## Purpose
Find candidate Upwork opportunities from approved sources.
...
A Steve-fit search for Klaviyo Shopify retention work finds a DTC beauty brand asking for flow rebuilds and suppresses generic 'email data entry' posts.
...
Recommended next skill: `job-extraction`. Include the output schema above so the next skill can continue without rereading broad context.
