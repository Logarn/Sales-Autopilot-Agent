...
## Purpose
Check operational health of worker, sources, Slack, DB, and daily summaries.
...
## Required inputs
health command output, logs, env/config state
...
A daily check sees Slack webhook configured and DB stats healthy; if Apify fails repeatedly, alert instead of silently skipping jobs.
...
Recommended next skill: `upwork-search or operator escalation`. Include the output schema above so the next skill can continue without rereading broad context.
