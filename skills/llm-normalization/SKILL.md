# LLM Normalization Skill

## Purpose
Normalize messy job details without changing facts or making paid calls unless explicitly enabled.
...
## Required inputs
raw/captured job text and existing parsed fields
...
A client writes 'need email flows fixed asap' and the skill normalizes it to Klaviyo flow optimization with urgency while preserving unknown budget.
...
Recommended next skill: `fit-scoring`. Include the output schema above so the next skill can continue without rereading broad context.
