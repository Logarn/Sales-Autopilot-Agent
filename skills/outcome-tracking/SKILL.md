...
## Purpose
Record application statuses, notes, submissions, and analytics.
...
## Required inputs
job id, status, notes, Connects/spend/rate/proof used
...
After Steve manually applies to a Klaviyo role, record required Connects, bid, selected attachments, reply/interview outcome.
...
Recommended next skill: `fit-scoring tuning`. Include the output schema above so the next skill can continue without rereading broad context.
