...
- Scheduled Upwork Best Matches discovery through the dedicated agent Chrome/CDP profile
...
Scheduled discovery is intended to run continuously in the final product. It should use jitter, dedupe aggressively, cap queued jobs per run, and skip when the browser/session is blocked, unhealthy, or when too many capture actions are already pending. Discovery itself does not spend Connects; the primary risks are queue spam, duplicate jobs, weak-fit jobs, browser instability, and Slack noise.
...
Backlog before scaling auto-prepare/draft-fill: build **LLM Job Intelligence Parser v0** to classify platform (Brevo, Klaviyo, Omnisend, Mailchimp, HubSpot, etc.), business type, ecommerce vertical, job category, task type, fit level, proposal angle, and platform mismatch risk.
